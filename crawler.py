    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            ui_keywords = [
                ...
                "이 키워드를 선택한 인원", "매장이 넓어요", "음식이 맛있어요" # 키워드 리뷰 관련 추가
            ]
            
            visit_info = ["방문일", "예약", "대기 시간", "목적", "동행", "안내", "메뉴"]
            skip_keywords = ui_keywords + visit_info
            
            # [중요] 수집 시작 플래그 (Marker)
            # '리뷰 클렌징' 또는 '최신순'이 나온 이후부터 수집 시작
            ...
            for el in elements:
                ...
                try:
                    ...
                    # 마커 확인 (수집 시작 시점 판단)
                    if "리뷰 클렌징" in text or "최신순" in text:
                    ...
                    # 6. 통계 수치 패턴 제외
                    if re.search(r'\d{1,3}(,\d{3})*[회명원개]', text): continue
                    if re.search(r'리뷰 \d+', text): continue
                    if re.search(r'사진 \d+', text): continue
