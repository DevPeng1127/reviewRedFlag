...
def crawl_naver_reviews(place_id: str, max_count: int = 50) -> List[Dict]:
    ...
    """
    reviews = []
    target_url = f"https://m.place.naver.com/place/{place_id}/review/visitor?entry=plt"
    
    print(f"🚀 크롤링 시작 (Target): {target_url}")

    with sync_playwright() as p:
        # [수정됨] headless=False로 설정하여 브라우저가 실제로 뜨게 함 (캡차 대응 및 디버깅 용)
        browser = p.chromium.launch(
            headless=False, 
            args=["--disable-blink-features=AutomationControlled"] # 자동화 탐지 회피
        )
        ...
        try:
            ...
            if "이용이 제한되었습니다" in page.content():
                print("🚫 여전히 차단된 상태입니다. 나중에 다시 시도해주세요.")
                return []
            ...
            skip_keywords = ["영수증", "주문", "길찾기", "공유", "신고", "업체", "소식", "이용이 제한되었습니다"]
